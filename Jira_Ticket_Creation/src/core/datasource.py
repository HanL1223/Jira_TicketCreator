...
from core.model import JiraIssue
...
class JsonlIssueSource(IssueSources):
...
class CSVISsueSource(IssueSources):
    def __init__(self,path:Path)-> None:
        self.path = path
    def iter_issues(self) -> Iterator[JiraIssue]:
...
def create_issue_source(config: SourceConfig) -> IssueSources:
