...
class ChromaVectorStore:
    """
    Thin wrapper around ChromaDB collection operations (add/query/get/count).
    Keeps your app code decoupled from Chroma-specific API details.
    """

    def __init__(self, config: ChromaConfig) -> None:
        self._config = config

        # Lazy import: Chroma is only required when you actually use the vector store.
        try:
            import chromadb  # type: ignore
        except ImportError as e:
            raise ImportError("Missing dependency: chromadb. Install it via uv.") from e
    ...
    # This is a function to add documents + embeddings into the Chroma collection.
    def add(
        self,
        ids: list[str],
        documents: list[str],
    ...
    # This is a function to run a similarity search against the collection using query embeddings.
    # It returns a Chroma-like dict whose keys match your retriever.py expectations.
    def query(
        self,
